...
func ListenWithOptions(laddr string, block BlockCrypt, dataShards, parityShards int) (*Listener, error) {
	// laddr: server端中 -l 指定的 地址 ":4000"
	// OverRide Here?

	// OverRide End //
	// udpaddr, err := net.ResolveUDPAddr("udp", ":"+s[len(s)-1])
	// udpaddr, err := net.ResolveUDPAddr("udp", ":40000") //测试
	// 原先的代码↓
...
func DialWithOptions(raddr string, block BlockCrypt, dataShards, parityShards int) (*UDPSession, error) {
	// raddr: 客户端中 -t 指定的 地址 "127.0.0.1:4000"
	// OverRide Here?
	// udpaddr, err := net.ResolveUDPAddr("udp", "127.0.0.1:40000") // 测试 单纯修改raddr是否会导致问题
	// OverRide End //
