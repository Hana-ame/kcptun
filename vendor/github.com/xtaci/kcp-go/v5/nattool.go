...
func GetAddrBrute(conn *net.UDPConn) (string, error) {
	s := make(chan string)
	e := make(chan error)
	go func() {
		flag := true
...
// changed packetconn to udpconn, wish it could work
// copy 111223
func GetAddr(conn *net.UDPConn) (string, error) {
...
// copy 111223
func GetAddress(conn *net.UDPConn, server string) (string, error) {
